...
//    2. K. Sugimoto and S. Kamata: "Efficient constant-time Gaussian filtering //       with sliding DCT/DST-5 and dual-domain error minimization", ITE Trans. //       Media Technol. Appl., vol. 3, no. 1, pp. 12-21 (Jan. 2015).
...
public:
	...
	void operator()(const cv::Mat_<double>& src,cv::Mat_<double>& dst,int tone=256)
	{
		...
		for(int k=1;k<=K;++k)
		{
			...
			gaussian.filter_xy(comps,comps);
