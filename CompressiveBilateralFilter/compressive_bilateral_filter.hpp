...
private:
	double s,kappa;
public:
	derivative_estimated_gaussian_range_kernel_error(double s,int K):s(s),kappa(M_PI*(2*K+1)){}
public:
...
inline std::pair<int,double> estimate_optimal_K_and_T(double s,double tau)
{
...
void apply_bilateral_filter_compressive(const cv::Mat_<double>& src,cv::Mat_<double>& dst,double ss,double sr,double tau)
{
	...
	std::pair<int,double> params=estimate_optimal_K_and_T(sr,tau);
	int K=params.first;
